...
# <span style="color:skyblue; font-weight:bold">TC-SEP-C002 — Storage Empty Pallet — Storage from HP Station</span>

| **Field**            | **Value**                                                                                                                                                                          |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Test Case ID**     | TC-SEP-C002-storage-empty-pallet-HP-Station                                                                                                                                        |
...
| **Physical Route**   | `ST1301 / ST1302` → `ID26` → `AutoStorageScheduler` → `ID05` (**DIRECT_TRAVEL**) → `720x BCR` → `ID26` → `ID05` (**STORAGE**) → `SRM Bin (9007–9010)` → `ID33` (Completion)        |
...
| **Station Operator** | `AsrsInboundStationOperator` at BCR 720x — FINAL BCR determination                                        |
...
SAP/User Screen
    ...
    ▼
AutoStorageScheduler  ◄── ID26 from AGC (pallet arrives at 130x)
    │  INSERT DNPallet, DNStock, DNCarryInfo (DIRECT_TRAVEL, dest=720x, end=9200)
    │  UPDATE DNStoragePlan STATUS→1, DNArrival SCHEDULED
    │  → ID05 (DIRECT_TRAVEL): 130x → 720x
    ▼
AGC/Conveyor transports pallet to BCR 720x
    │  → ID26 Arrival at 720x
    ▼
AsrsInboundStationOperator (720x)
...
| 2   | AGC connection: `DMStation.connection_flag = 1` (**CONNECTED**), `status_flag = 1` (**NORMAL**) for stations 130x and 720x | `SELECT station_no, status_flag, connection_flag FROM dmstation WHERE station_no IN ('1301','1302','7207','7208','7209','7210')`                                |
...
| 8   | `DMRouteId` routes exist from stations `ST1301/ST1302` to 720x BCRs (**7207–7210**)                                        | `SELECT COUNT(*) FROM dmrouteid WHERE start_station_no IN ('1301','1302') AND end_station_no LIKE '72_'` → Expected: ≥ 8                                        |
| 10  | BCR stations **7207–7210**: `sendable = 1`, `max_instruction = 2`, `class_name = AsrsInboundStationOperator`               | `SELECT station_no, sendable, max_instruction, class_name FROM dmstation WHERE station_no BETWEEN '7207' AND '7210'`                                            |
...
| **Field**                | **Value**      | **Notes**                                             |
| ------------------------ | -------------- | ----------------------------------------------------- |
| **Pallet No**            | `EMP0000002`   | Must be ≥ 10 characters (PALLET_NO_LENGTH constraint) |
...
| **HP Inbound BCR Range** | `7207–7210`    | HP ASRS Inbound BCR — FGW1 Ambient                    |
| **HP Inbound Station**   | `1301`, `1302` | HP Inbound station                                    |
...
| ----- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ----------------- | --------------- |
| 1.1   | Ensure all pre-conditions are met. Enter `EMP0000002` in **Pallet No** and click **Set (F2)**. | Success message `6461009` displayed. Screen input field clears after successful registration.                          |                   | [ ] P <br>[ ] F |
...
| `BCR_DATA`          | `EMP0000002`                               | Value from screen Pallet No field                                      | [ ] P [ ] F     |
...
**Purpose:** Verify that when a physical pallet with barcode `EMP0000002` arrives at inbound station **1301** (or **1302**), `AutoStorageScheduler` correctly matches the `DNSTORAGEPLAN`, selects the appropriate BCR, and creates all carry-related DB records in a single atomic transaction.

**AGC Reference:** AGC Linkage Spec Flow **05** — Steps `25–27` (HP side arrival and transport command).

| **#** | **Action**                                                                                                                                                  | **Expected Result**                                                                                                                                                                                                                                                                                                                   | **Actual Result** | **Status**      |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | --------------- |
| 2.1   | Physical/simulated pallet with BCR `EMP0000002` placed at inbound station `1301` / `1302`. AGC reads the barcode and sends **ID26** (Arrival Report) to MC. | AGC sends ID26 with: `MC key = All "9"` (**DUMMY**), `Load information = "1" (Present)`, `BC data = EMP0000002`, `Control Information = "000"`. `InOutStationOperator` inserts `DNARRIVAL`: `station_no = 1301/1302`, `carry_key = DUMMY`, `sch_flag = 0` (**NOT_SCHEDULED**).                                                        |                   | [ ] P <br>[ ] F |
| 2.2   | Allow `AutoStorageScheduler` polling cycle to execute.                                                                                                      | Scheduler finds `DNSTORAGEPLAN` with `bcr_data = 'EMP0000002'`, `status_flag = 0`, `plan_area_no = 9200`. `validateStoragePlan()` passes: Zone `004` (**EMP_PB**) is allowed at station **1301** / **1302**.                                                                                                                          |                   | [ ] P <br>[ ] F |
| 2.3   | Verify BCR selection logic (Tier 1 — `selectAisleAndGetBcrStation()`).                                                                                      | Zone **004** maps to aisles 9007–9010. `getReachableBcrForAisle()` checks `DMRouteId` from source **1301** / **1302**: route `1301/1302 → 720x` exists. Selected BCR is one of `7207`, `7208`, `7209`, or `7210` (least busy or round-robin).                                                                                         |                   | [ ] P <br>[ ] F |
...
| 2.5   | Verify `DNPALLET` is inserted.                                                                                                                              | Run **SQL-02a**. One row exists: `current_station_no = '1301/1302'`, `item_code = EMP_PB`, `soft_zone_id = '004'`, `status = not STORED (in transit)`.                                                                                                                                                                                |                   | [ ] P <br>[ ] F |
...
| 2.7   | Verify `DNCARRYINFO` is inserted.                                                                                                                           | Run **SQL-02c**. One row: `carry_flag = '3'` (**DIRECT TRAVEL**), `dest_station_no = '720x'` (selected BCR), `end_station_no = '9200'`.                                                                                                                                                                                               |                   | [ ] P <br>[ ] F |
...
| 2.10  | Verify `StorageSender` dispatches ID05.                                                                                                                     | MC sends **ID05** (Transport Command) to AGC: `Transport classification = "3" (Direct transfer)`, `Source station = 1301/1302`, `Destination station = 720x (e.g. 7208)`, `Location number = All "0"`, `Load size = 000`. AGC responds with **ID25** `Response section = "00" (Normal)`. `DNARRIVAL.send_flag` updated to **SENDED**. |                   | [ ] P <br>[ ] F |

---

### Case 3 : AGC Transport: 130x → BCR 720x (ID26 at BCR)
...
| **#** | **Action**                                                                                                                             | **Expected Result**                                                                                                                                                                        | **Actual Result** | **Status**      |
| ----- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | --------------- |
| 3.1   | AGC begins STV transport of pallet from station **1301** / **1302** toward BCR **720x**.                                               | AGC sends **ID64** (Pick-up completion report) once STV picks up pallet at P&D station. MC acknowledges.                                                                                   |                   | [ ] P <br>[ ] F |
| 3.2   | Pallet travels via conveyor and arrives at the BCR inlet of the selected BCR (e.g., 7208). AGC sends **ID26** (Arrival Report) at BCR. | ID26 received at MC: `MC key = Number assigned by MC (existing carry key)`, `Station number = 7208`. `AsrsInboundStationOperator.arrival()` triggered at station 7208.                     |                   | [ ] P <br>[ ] F |
| 3.3   | Verify FINAL BCR determination in `AsrsInboundStationOperator`.                                                                        | `wh_station_no` for station **7208** = `9200`. `end_station_no` in `DNCARRYINFO` = `9200`. Since `wh_station == end_station` → classified as **FINAL BCR** (NOT intermediate).             |                   | [ ] P <br>[ ] F |
| 3.4   | Verify `DNCARRYINFO` update at FINAL BCR.                                                                                              | `carry_flag` updated: `3` (**DIRECT TRAVEL**) → `1` (**STORAGE**). `dest_station_no` updated to `9200` (warehouse station). `source_station_no` updated to `7208`. `cmd_status` → `START`. |                   | [ ] P <br>[ ] F |
...
**Purpose:** Verify `StorageSender` at BCR **720x** selects an empty shelf in Zone **004** (aisles 9007–9010), reserves it, and dispatches the final storage ID05 to AGC.

| **#** | **Action**                                                                                | **Expected Result**                                                                                                                                                                                                                                                                                                         | **Actual Result** | **Status**      |
| ----- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | --------------- |
| 4.1   | `StorageSender` runs `soueceRightStation()` capacity check for BCR **720x**.              | `DMStation.max_instruction = 2`, suspend = 0, mode = NORMAL → BCR is eligible to receive a storage command.                                                                                                                                                                                                                 |                   | [ ] P [ ] F     |
...
| 5.10  | Verify `DNCARRYINFO` deletion (Order 9).                                 | `DNCARRYINFO` row for `BCR_DATA = 'EMP0000002'` is **deleted**. No orphan carry records remain.                                                                                                                                                 |                   | [ ] P <br>[ ] F |
...
| **#** | **Action**                                                                                                                  | **Expected Result**                                                                                                                                                                                           | **Actual Result** | **Status**      |
| ----- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- | --------------- |
| 7.1   | Trigger ID35 while pallet is at inbound station **1301** / **1302** with carry already created (`sch_carry_key` populated). | `StorageTrackingDeleter` is called. `DNCARRYINFO` deleted. `DNPALLET` deleted. `DNSTOCK` deleted. `DNARRIVAL` deleted. `DNSTORAGEPLAN` status reset to `0` (not deleted).                                     |                   | [ ] P <br>[ ] F |
| 7.2   | Trigger ID35 while pallet is in transit at BCR **720x** (carry exists with DIRECT_TRAVEL or STORAGE flag).                  | `StorageTrackingDeleter` deletes all records: `DNCARRYINFO`, `DNPALLET`, `DNSTOCK`, `DNWORKINFO`, `DNARRIVAL`. `DNSTORAGEPLAN` status reset to `0`. `DMShelf` status reset to `0` (EMPTY) if it was RESERVED. |                   | [ ] P <br>[ ] F |
...
-- DEST_STATION_NO  = '720x' (e.g. 7207, 7208, 7209, or 7210)
-- END_STATION_NO   = '9200'
```
...
FROM dmshelf
WHERE location_no = (
    SELECT location_no FROM dnstock WHERE bcr_data = 'EMP0000002'
);
...
| `bcr_data`           | `EMP0000002`               | Physical pallet barcode                                                                   | [ ] P <br>[ ] F |
...
| -------- | ------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| **ID26** | AGC → MC      | Pallet arrives at station 1301/1302           | `MC key = "9999..."` (DUMMY), `BC data = EMP0000002`, `Load info = "1"`, `Control info = "000"`      |
| **ID05** | MC → AGC      | After `AutoStorageScheduler` creates carry    | `Transport class = "3"` (Direct transfer), `Source = 1301/1302`, `Dest = 720x`, `Location = All "0"` |
...
| **ID26** | AGC → MC      | Pallet arrives at BCR 720x                    | `MC key = assigned carry key`, `Station = 720x`                                                      |
| **ID05** | MC → AGC      | After `StorageSender` selects bin             | `Transport class = "1"` (Storage), `Source = 720x`, `Dest = 9007–9010`, `Location = bin address`     |
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| 1   | `DNSTORAGEPLAN.STATUS_FLAG = 4` (COMPLETE) for `BCR_DATA = 'EMP0000002'`                                                                                      | SQL-04                                                                   |
